...
export class UserConnectionsService {
  constructor(private prisma: PrismaService, private profile: UserProfileService) {}
  ...
  async followUser(userId: string, targetId: string) {
    ...
    const [user, target] = await Promise.all([
      this.profile.findUser(userId),
      this.profile.findUser(targetId),
    ])
    if (!user || !target) return { message: 'User not found' }
  ...
  async unfollowUser(userId: string, targetId: string) {
    await this.prisma.resFollow.deleteMany({ where: { follower_id: userId, following_id: targetId } })
    return { message: `User ${userId} unfollowed ${targetId}` }
  }

  async removeFollower(userId: string, followerId: string) {
    await this.prisma.resFollow.deleteMany({ where: { follower_id: followerId, following_id: userId } })
  ...
  private async attachFollowStatus(currentUserId: string, users: any[]): Promise<UserConnectionDto[]> {
  ...
  async getConnections(userId: string, type: 'followers' | 'following' | 'friends', search?: string) {
    ...
    if (search) {
      ...
      result.users = result.users.filter(u => u.nickname.toLowerCase().includes(q) || u.id.toLowerCase().includes(q))
    }
