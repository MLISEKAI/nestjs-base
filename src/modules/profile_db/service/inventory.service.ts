...
export class InventoryService {
  ...
  async addInventoryItem(userId: string, dto: CreateInventoryItemDto) {
    return this.prisma.resInventory.create({ data: { user_id: userId, item_id: dto.item_id, quantity: dto.quantity ?? 1 } });
  }

  async updateInventoryItem(userId: string, itemId: string, dto: UpdateInventoryItemDto) {
    const existing = await this.prisma.resInventory.findFirst({ where: { id: itemId, user_id: userId } });
    if (!existing) throw new NotFoundException('Inventory item not found');
    return this.prisma.resInventory.update({ where: { id: itemId }, data: { quantity: dto.quantity ?? existing.quantity } });
  }
