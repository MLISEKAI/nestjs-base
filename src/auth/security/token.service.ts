...
export class TokenService {
  ...
  constructor(
    ...
    this.jwtSecret =
      this.configService.get<string>('jwt.secret') || process.env.JWT_SECRET || 'secretKey';
    ...
    this.refreshTtlMs =
      this.configService.get<number>('jwt.refreshTtlMs') ?? 1000 * 60 * 60 * 24 * 30;
  ...
  async blacklistAccessToken(token: string | null | undefined, userId: string, reason?: string) {
    ...
    try {
      ...
      const expiresAt = payload.exp
        ...
        : new Date(Date.now() + this.accessTtlSeconds * 1000);
  ...
  private async resolveRefreshToken(
    refreshToken: string,
  ): Promise<ResRefreshToken & { user: ResUser }> {
